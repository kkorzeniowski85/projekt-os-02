 ...
 * podmienionych albo skasowanych (np. głoski z doklejoną samogłoską). Bez
 * tego urządzenie grałoby na zawsze pierwszą pobraną wersję nagrania.
 *
 * NAGRANIA: element <audio> pyta o fragmenty pliku (nagłówek Range), GitHub
 * Pages odpowiada wtedy 206, a takiej odpowiedzi Cache API nie przyjmuje.
 * Dlatego nagranie pobieramy w całości (200), zapisujemy pod samym adresem i
 * fragment dla odtwarzacza wycinamy sami. HEAD (sprawdzenie, czy nagranie
 * istnieje) odpowiadamy z manifestu, bez sieci i bez pobierania pliku —
 * panel rodzica sprawdza tak ponad tysiąc nagrań przy każdym otwarciu.
 */
 ...
 * działające łącze gra przy pierwszym stuknięciu, choćby duża rymowanka szła
 * kilkanaście sekund. Łącze, które wisi, daje po tym czasie błąd (nie zawsze
 * gra wtedy zapas — rymowanka po prostu milknie), a plik dalej pobiera się
 * w tle na następny raz.
 */
...
/** „audio/phonemes/sh.wav" — nazwa nagrania tak jak w deploy.json. */
...
/** Kopia TEGO SAMEGO adresu; „/rodzic" i „/rodzic/" to ta sama strona. */
