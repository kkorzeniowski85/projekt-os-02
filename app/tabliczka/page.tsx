...
import { BigButton, Card, PageHeader, ParentTip, STATUS_STYLE } from "@/components/akademia/ui";
import { LEARNING_ORDER, MTC } from "@/lib/akademia/curriculum/tables";
import { HEROES_BY_MODULE } from "@/lib/akademia/heroes";
import { useProgress } from "@/lib/akademia/progress/store";
import { unitKeyOf } from "@/lib/akademia/progress/types";
import { focusTable, tablesSummary } from "@/lib/akademia/tables/practice";
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_1fr]">
        <Card className="flex flex-col gap-4">
          ...
          <div className={`h-3 w-full overflow-hidden rounded-full bg-white/10 ${ready ? "" : "animate-pulse"}`}>
            <div
        ...
        <Card className="flex flex-col gap-3">
          ...
          <p className="text-sm text-paper/75">
            ...
            pytania na rozgrzewkę, bez podpowiedzi. Wynik na 25.
          </p>
          {!ready ? (
            // Do wczytania postępu nie wiadomo, czy test już był — bez fałszywego
            // „Jeszcze nie było”. Wysokość jak wiersz „Ostatni wynik”.
            <p className="text-lg text-paper/40" aria-hidden>
              …
            </p>
          ) : lastMock ? (
            <p className="text-lg">
      ...
      <section className="flex flex-col gap-3">
        ...
        <p className="text-sm text-paper/70">
          Od najłatwiejszych tabliczek do najtrudniejszych. Lekcja uczy liczenia skokami i słów „lots of”,
          „groups of”, „times” — trening potem utrwala fakty.
        </p>
      ...
      <Card>
        ...
        {/* Przed wczytaniem postępu mapa pustego profilu („jeszcze nie ćwiczone”
            w każdej kratce) byłaby nieprawdziwa — zostaje tylko jej miejsce. */}
      ...
      <ParentTip title="Jak to działa (dla rodzica)">
        <p className="mb-2">
          ...
          kłopot — od razu. Nowe fakty dochodzą tylko wtedy, gdy nie piętrzą się słabe (wyjątek: trening
          jednej tabliczki ma zawsze co najmniej 6 różnych faktów, więc w prawie nieruszonej tabliczce
          dokłada kilka nowych).
        </p>
        <p>
        ...
        <p className="mt-2">
          Kolejność tabliczek idzie za angielskim programem: najpierw ×2, ×10 i ×5, potem ×3, ×4 i ×8, na
          końcu ×11, ×6, ×9, ×7 i ×12. ×7 i ×12 są ostatnie, bo wnoszą najmniej nowych faktów — resztę
          dziecko zna już z drugiej strony (7 × 8 = 8 × 7). Trening wprowadza nowe fakty tylko z jednej
          tabliczki naraz. Przed nową tabliczką misja dnia i ekran treningu proponują jej lekcję „Liczymy
          co…”; trening, który skończył poprzednią tabliczkę, nie zaczyna następnej tego samego dnia bez
          tej lekcji (dalsze treningi tego dnia to powtórki), a od następnego dnia zaczyna ją i bez niej.
        </p>
      </ParentTip>
    </div>
  );
