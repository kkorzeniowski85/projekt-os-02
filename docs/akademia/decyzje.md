...
> Kopia `docs/decyzje.md` z repozytorium Akademii (`szkola/`) z chwili
> połączenia z Ligą. Ścieżki w tekście (`lib/…`, `components/…`) dotyczą
> tamtego repozytorium — tutaj kod leży w `lib/akademia/…` i
> `components/akademia/…` (patrz [../polaczenie.md](../polaczenie.md)).
