/**
 * Audyt treści Akademii — uruchamiany przed publikacją (`npm run audit:akademia`).
 *
...
import { buildMtcForm, checkMtcForm, TABLES } from "../../lib/akademia/curriculum/tables.ts";
import { numbersWithAudio } from "../../lib/akademia/curriculum/numbers.ts";
import { audioSlug } from "../../lib/akademia/curriculum/slug.ts";
import { MATHS_TOPICS, mathsPhrases, OPERATIONS, WORD_PROBLEMS } from "../../lib/akademia/curriculum/maths.ts";
import { READING_TEXTS, buildReadingSession, readingPhrases } from "../../lib/akademia/curriculum/reading.ts";
import { CLASSROOM_UNITS, classroomPhrases } from "../../lib/akademia/curriculum/classroom.ts";
import { buildCountingSession, buildTrainingSession, countingPhrases } from "../../lib/akademia/tables/sessions.ts";
import { normalizeWord, soundsOf } from "../../lib/akademia/session/exercise.ts";
...
// 7. Pliki nagrań
const audioDir = path.join(root, "public", "audio", "akademia");
if (existsSync(audioDir)) {
  ...
  if (missingFiles.length) warn.push(`brakuje ${missingFiles.length} plików nagrań (npm run audio:akademia), np. ${missingFiles.slice(0, 5).join(", ")}`);
  ...
  warn.push("nie ma jeszcze katalogu public/audio/akademia (npm run audio:akademia)");
}
